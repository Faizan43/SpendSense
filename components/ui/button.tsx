 ...
 * we style a link as a button (`render={<Link href=… />}`), it must be told —
 * otherwise it applies button semantics to an anchor, which breaks keyboard
 * behaviour and screen-reader announcement. Detecting an `href` on the rendered
 * element handles that once here instead of at every call site.
 */
function rendersAnchor(render: unknown): boolean {
  if (!isValidElement(render)) return false
  const props = render.props as { href?: unknown } | null
